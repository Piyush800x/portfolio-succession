...
import { hackathonProjects } from "@/app/lib/data";

export default function HackathonProjects() {
  return (
    <SectionWrapper id="hackathons">
      <SectionHeading title="Hackathon Projects" subtitle="Competitive Innovation" />

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
        {hackathonProjects.map((project, i) => (
          <HackathonProjectCard key={project.slug} project={project} index={i} />
        ))}
      </div>
    </SectionWrapper>
  );
      ...
      <Link href={`/projects/${project.slug}`} className="block group h-full">
        <div className="relative bg-bg-card border border-border hover:border-border-gold rounded-sm p-8 md:p-10 transition-all duration-700 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(180,147,80,0.08)] h-full flex flex-col">
          ...
          <div className="flex flex-wrap gap-1.5 mb-6">
            {project.team && (
              <span className="text-[9px] tracking-[0.15em] uppercase text-gold border border-gold-muted/30 rounded-sm px-2 py-0.5 bg-gold/5">
                Team: {project.team}
              </span>
            )}
